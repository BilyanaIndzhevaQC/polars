...
use sqlparser::ast::{
    ...
    Expr as DBExpr, Function as SQLFunction, JoinConstraint, OrderByExpr, SelectItem,
...
pub(crate) struct DBExprVisitor<'a> {
    ctx: &'a SQLContext,
}

impl DBExprVisitor<'_> {
    fn visit_expr(&self, expr: &DBExpr) -> PolarsResult<Expr> {
        match expr {
            DBExpr::AllOp(_) => Ok(self.visit_expr(expr)?.all(true)),
            DBExpr::AnyOp(expr) => Ok(self.visit_expr(expr)?.any(true)),
            DBExpr::ArrayAgg(expr) => self.visit_arr_agg(expr),
            DBExpr::Between {
                expr,
                negated,
                low,
            ...
            DBExpr::BinaryOp { left, op, right } => self.visit_binary_op(left, op, right),
            DBExpr::Cast { expr, data_type } => self.visit_cast(expr, data_type),
            DBExpr::Ceil { expr, .. } => Ok(self.visit_expr(expr)?.ceil()),
            DBExpr::CompoundIdentifier(idents) => self.visit_compound_identifier(idents),
            DBExpr::Floor { expr, .. } => Ok(self.visit_expr(expr)?.floor()),
            DBExpr::Function(function) => self.visit_function(function),
            DBExpr::Identifier(ident) => self.visit_identifier(ident),
            DBExpr::InList {
                expr,
                list,
                negated,
            } => self.visit_is_in(expr, list, *negated),
            DBExpr::IsDistinctFrom(e1, e2) => {
                Ok(self.visit_expr(e1)?.neq_missing(self.visit_expr(e2)?))
            },
            DBExpr::IsNotDistinctFrom(e1, e2) => {
                Ok(self.visit_expr(e1)?.eq_missing(self.visit_expr(e2)?))
            },
            DBExpr::IsFalse(expr) => Ok(self.visit_expr(expr)?.eq(lit(false))),
            DBExpr::IsNotFalse(expr) => Ok(self.visit_expr(expr)?.eq(lit(false)).not()),
            DBExpr::IsNotNull(expr) => Ok(self.visit_expr(expr)?.is_not_null()),
            DBExpr::IsNotTrue(expr) => Ok(self.visit_expr(expr)?.eq(lit(true)).not()),
            DBExpr::IsNull(expr) => Ok(self.visit_expr(expr)?.is_null()),
            DBExpr::IsTrue(expr) => Ok(self.visit_expr(expr)?.eq(lit(true))),
            DBExpr::Nested(expr) => self.visit_expr(expr),
            DBExpr::Trim {
                expr,
                trim_where,
                trim_what,
            } => self.visit_trim(expr, trim_where, trim_what),
            DBExpr::UnaryOp { op, expr } => self.visit_unary_op(op, expr),
            DBExpr::Value(value) => self.visit_literal(value),
            e @ DBExpr::Case { .. } => self.visit_when_then(e),
            other => {
    ...
    fn visit_unary_op(&self, op: &UnaryOperator, expr: &DBExpr) -> PolarsResult<Expr> {
    ...
    fn visit_cast(&self, expr: &DBExpr, data_type: &SQLDataType) -> PolarsResult<Expr> {
    ...
    fn visit_is_in(&self, expr: &DBExpr, list: &[DBExpr], negated: bool) -> PolarsResult<Expr> {
        let expr = self.visit_expr(expr)?;
        let list = list
            .iter()
            .map(|e| {
                if let DBExpr::Value(v) = e {
    ...
    fn visit_when_then(&self, expr: &DBExpr) -> PolarsResult<Expr> {
        if let DBExpr::Case {
            operand,
            conditions,
            results,
...
pub(crate) fn parse_sql_expr(expr: &DBExpr, ctx: &SQLContext) -> PolarsResult<Expr> {
    let visitor = DBExprVisitor { ctx };
...
pub(super) fn process_join_constraint(
    ...
    if let JoinConstraint::On(DBExpr::BinaryOp { left, op, right }) = constraint {
        match (left.as_ref(), right.as_ref()) {
            (DBExpr::CompoundIdentifier(left), DBExpr::CompoundIdentifier(right)) => {
            ...
            (DBExpr::Identifier(left), DBExpr::Identifier(right)) => {
